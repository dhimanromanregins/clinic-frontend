...
const AllLabs = ({ navigation }) => {
  ...
  const SickLeaveHistory = language === 'en' ? 'Lab Documents' : 'تاريخ الإجازات المرضية';
  ...
  const fetchParentSickLeaveData = async () => {
    try {
      ...
      const response = await fetch(`${BASE_URL}/api/documents/parent/lab_report`, {
        method: 'GET',
        headers: {
