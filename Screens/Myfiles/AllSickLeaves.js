...
const AllSickLeaves = ({ navigation }) => {
  ...
  const MedicalReports = language === 'en' ? 'Sick Leaves' : 'التقارير الطبية';
  ...
  const fetchSickLeaveData = async () => {
    try {
      ...
      const response = await fetch(`${BASE_URL}/api/documents/parent/sick_leave`, {
        method: 'GET',
        headers: {
