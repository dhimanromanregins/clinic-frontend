...
const AllParentSickLeaves = ({ navigation }) => {
  ...
  const SickLeaveHistory = language === 'en' ? 'Parents Sick Leave History' : 'تاريخ الإجازات المرضية';
  ...
  const fetchParentSickLeaveData = async () => {
    try {
      ...
      const response = await fetch(`${BASE_URL}/api/documents/parent/parent_sick_leave`, {
        method: 'GET',
        headers: {
...
export default AllParentSickLeaves;
