...
const ParentSickLeaveHistory = ({ navigation,route }) => {
  ...
  const SickLeaveHistory = language === 'en' ? 'Sick Leave History' : 'تاريخ الإجازات المرضية';
  ...
  const fetchParentSickLeaveData = async () => {
    try {
      ...
      const response = await fetch(`${BASE_URL}/api/documents/child/${childId}/parent_sick_leave/`, {
        method: 'GET',
        headers: {
...
export default ParentSickLeaveHistory;
